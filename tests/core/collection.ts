...
import { FEES_WALLET, findConverterPda, getTokenAccount } from "../helpers/pdas"
...
describe("Core - With collection", () => {
  ...
  it("Can create a new converter", async () => {
    ...
    assert.equal(collection.updateAuthority, authority.publicKey, "Expected authority to be set to authority")
    assert.equal(
      collection.updateDelegate.authority.address,
      converter,
      "Expected update delegate to be set to converter"
    )
