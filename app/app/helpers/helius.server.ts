...
export async function getCollectionsPageForUa(ua: string, page: number) {
  try {
    const result = await client.rpc.searchAssets({
      authorityAddress: ua,
      page,
...
export async function getCollectionsForUa(ua: string) {
  const nfts = []
  let page = 1
  let result
  while ((result = await getCollectionsPageForUa(ua, page++))?.items.length) {
    nfts.push(...result.items)
  }
...
export async function getDaForCollection(collection: string) {
  const da = await client.rpc.getAssetsByGroup({
    groupKey: "collection",
    groupValue: collection,
    limit: 1,
    page: 1,
